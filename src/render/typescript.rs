...
impl TypeScriptRenderer {
    ...
    fn rand_type() -> &'static str {
        let types = ["number", "string", "boolean", "any", "unknown", "void", "never", "object"];
    ...
    fn render_stmt(&mut self, stmt: &Statement, out: &mut String) {
        match stmt {
            Statement::Assignment(name, expr) => {
                let kw = ["let", "const"][rand::rng().random_range(0..2usize)];
                self.w.write_line(
                    out,
                    &format!("{} {}: {} = {};", kw, name, Self::rand_type(), self.render_expr(expr)),
                );
            }
            ...
            Statement::FunctionDef(name, _params, body) => {
                self.w.write_line(
                    out,
                    &format!("function {}(): {} {{", name, Self::rand_type()),
                );
                self.w.inc();
                self.render_block(body, out);
                self.w.dec();
